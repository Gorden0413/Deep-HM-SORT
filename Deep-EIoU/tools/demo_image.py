...
def imageflow_demo(predictor, extractor, vis_folder, current_time, args):
    ...
    if osp.isdir(args.path):
        logger.info(f"Image sequence save_path is {save_folder}")
    else:
        logger.info(f"Video save_path is {save_path}")
        vid_writer = cv2.VideoWriter(
            save_path, cv2.VideoWriter_fourcc(*"mp4v"), frame_rate, (int(width), int(height))
        )

    tracker = Deep_HM_SORT(args, frame_rate=frame_rate)
    timer = Timer()
    frame_id = 0
    results = []
    
    for frame_id, frame_path in enumerate(image_paths if osp.isdir(args.path) else iter(lambda: cap.read()[1], None)):
        if frame_id % 30 == 0:
        ...
        if outputs[0] is not None:
            ...
            scale = min(1440 / frame.shape[1], 800 / frame.shape[0])
            det /= scale
            rows_to_remove = np.any(det[:, 0:4] < 1, axis=1)
            det = det[~rows_to_remove]
            cropped_imgs = [frame[max(0, int(y1)):min(frame.shape[0], int(y2)), max(0, int(x1)):min(frame.shape[1], int(x2))] for x1, y1, x2, y2, _, _, _ in det]
        ...
        if args.save_result:
            if osp.isdir(args.path):
                cv2.imwrite(osp.join(save_folder, f"{frame_id:06d}.jpg"), online_im)
            else:
