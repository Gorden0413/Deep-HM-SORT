...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat=None):
        ...
        # Deep HM-SORT: Keep all trajectory features and their timestamps
        self.features = []
        self.times = []  # Add this line to store feature timestamps
        self.alpha = 0.9
    ...
    def update(self, new_track, frame_id):
        ...
        if new_track.curr_feat is not None:
            self.update_features(new_track.curr_feat)
            self.features.append(new_track.curr_feat)
            self.times.append(frame_id)  # Store the timestamp for each feature
...
class Deep_HMSORT(object):
    def __init__(self, args, frame_rate=30):
        ...
        # Deep HM-SORT: No buffer size limit, keep all trajectories
        self.kalman_filter = KalmanFilter()

        self.proximity_thresh = args.proximity_thresh
        self.appearance_thresh = args.appearance_thresh

    def calculate_harmonic_cost(self, ious_dists, emb_dists, ious_mask):
        """
        Calculate harmonic mean of IoU and embedding distances
        """
        ...
        emb_dists[ious_mask] = 1.0
        
        # Calculate harmonic mean
        harmonic_dists = 2 * (ious_dists * emb_dists) / (ious_dists + emb_dists + eps)
        
        return harmonic_dists
    ...
    def update(self, output_results, embedding):
        ...
        # Multi-scale matching with harmonic mean cost
        for expand_scale in [0.7, 0.8]:
            ious_dists = matching.eiou_distance(strack_pool, detections, expand_scale)
            ious_dists_mask = (ious_dists > self.proximity_thresh)

            if self.args.with_reid:
                emb_dists = matching.embedding_distance(strack_pool, detections)
                # Deep HM-SORT: Use harmonic mean instead of minimum
                dists = self.calculate_harmonic_cost(ious_dists, emb_dists, ious_dists_mask)
            else:
                dists = ious_dists
        ...
        # Second association with harmonic mean cost
        if self.args.with_reid and len(detections_second) > 0:
        ...
        if self.args.with_reid:
            ...
            # Deep HM-SORT: Use harmonic mean for unconfirmed tracks
            dists = self.calculate_harmonic_cost(ious_dists, emb_dists, ious_dists_mask)
        else:
            dists = ious_dists
        ...
        # Deep HM-SORT: Keep all trajectories, no time limit for lost tracks
